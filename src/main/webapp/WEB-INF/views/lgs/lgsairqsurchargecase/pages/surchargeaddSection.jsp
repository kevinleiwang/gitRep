...
<% Date nowDate = new Date(); %>
   <form id="inquiry_quote_add_form" name="inquiry_quote_add_form" valid
		action="${ctx}/lgsairqsurchargecase/addListSection" method="POST">
		<sunivo:flushMessage />
		
		<table class="table table-bordered table-striped table_vam" id="dt_gal">
		<tr> 
			<td><span style="line-height:44px;">航空公司：</span>
				<!-- <g:logisticsCompanySelect  cusLogisticType="07" name="carrier" style="width:150px; " required="true"/> -->
				<sunivo:logisticsCompanySelect clazz="nochosen" cusLogisticType="07" name="carrier" style="width:150px; " required="true" />
			</td>
			<td><span style="line-height:44px;">价格有效期：</span><%-- <div id="expiryDate" class="input-append date datetime-picker">
	                      ...
	                      <div class="input-append date datetime-picker" style="margin:0px; " date-format="yyyy-mm-dd hh:ii:mm" id="expiryDate"
											...
											<input class="m-wrap" type="text" 
												value="<fmt:formatDate value='${expiryDate}' pattern='yyyy-MM-dd HH:mm:ss'/> "
												id="expiryDate" name="expiryDate" style="width:120px; height: 20px; "
												readonly>
		...
		<table class="table table-bordered table-striped table_vam" id="dt_gal">
			<thead>
				...
				<tr>
					...
					<td nowrap="nowrap">
						<input  onclick="inputMoney(this);" onblur="checkMoney(this);" type="text" value="" id="entities[${status.index}].volumeCost" name="entities[${status.index}].volumeCost" style="width: 30px;"/>
					</td>			
				</tr>
				</c:forEach>
