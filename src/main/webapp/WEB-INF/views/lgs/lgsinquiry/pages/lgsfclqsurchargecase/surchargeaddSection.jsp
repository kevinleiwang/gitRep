...
<div class="row${fluid}">
   <form id="inquiry_quote_add_form" name="inquiry_quote_add_form" valid
		...
		<tr> 
			<td  style="width:120;">船公司:<g:logisticsCompanySelect  cusLogisticType="01" name="carrier" required="true"/></td>
		...
		<tr> 
			<td>船程:<input type="text" name="estimatedRange" value="" /></td>
			...
			<thead>
				...
				<tr>
					...
					<td nowrap="nowrap" width="30">
						<select id="entities[${status.index}].ifAllin" name="entities[${status.index}].ifAllin" style="width: 60px;">
					...
					<td nowrap="nowrap">
						<input <c:if test="${'5' ne prdQuote.unitId}">onclick="inputMoney(this);" onblur="checkMoney(this);"</c:if>
						   type="text" value="" id="entities[${status.index}].nonContainerCost" name="entities[${status.index}].nonContainerCost" style="width: 30px;" <c:if test="${'5' eq prdQuote.unitId}">readonly="readonly"</c:if>  />
					</td>
					<td nowrap="nowrap">
						<input <c:if test="${'5' eq prdQuote.unitId}">onclick="inputMoney(this);" onblur="checkMoney(this);"</c:if>
								type="text" value="" id="entities[${status.index}].cost20Gp" name="entities[${status.index}].cost20Gp" style="width: 30px;" <c:if test="${'5' ne prdQuote.unitId}">readonly="readonly"</c:if>  />
					</td>
					<td nowrap="nowrap">
						<input <c:if test="${'5' eq prdQuote.unitId}">onclick="inputMoney(this);" onblur="checkMoney(this);"</c:if>
								type="text" value="" id="entities[${status.index}].cost40Gp" name="entities[${status.index}].cost40Gp" style="width: 30px;" <c:if test="${'5' ne prdQuote.unitId}">readonly="readonly"</c:if>  />
					</td>
					<td nowrap="nowrap">
						<input <c:if test="${'5' eq prdQuote.unitId}">onclick="inputMoney(this);" onblur="checkMoney(this);"</c:if>
						 		type="text" value="" id="entities[${status.index}].cost40Hq" name="entities[${status.index}].cost40Hq" style="width: 30px;" <c:if test="${'5' ne prdQuote.unitId}">readonly="readonly"</c:if> />
					</td>
